...
{
    private IObjectPool<Object> ChoosePool;
    [SerializeField] GameObject choose_element;
    [SerializeField] GameObject choose_child;
    ...
    private void Awake()
    {
        ChoosePool = new ObjectPool<Object>(InstantiateObject, OnObject, OnReleased);
    }
        ...
        foreach(Transform child in parent)
        {
            try
            {
                if(child.gameObject.activeSelf)
                    ChoosePool.Release(child.gameObject);
            }
            catch (System.Exception e) { Debug.Log(e); }
        }
        choose_child.gameObject.SetActive(false);
    }

    public void open()
    {
         choose_child.gameObject.SetActive(true);
    }
    public void hide()
    {
        choose_child.gameObject.SetActive(false);
    ...
    public void start_choose(int type)
    {
        if (type == 1)
        {
            ...
            {
                GameObject element = (GameObject)ChoosePool.Get();
                element.GetComponent<Choose_element>().set_param(book.Key, book.Value, this, type, null, null, null);
            }
        }
        else if (type == 2)
        {
            ...
            foreach (KeyValuePair<string, Dictionary<int, string[]>> topic in questions_book)
            {
                topic_count++;
                GameObject element = (GameObject)ChoosePool.Get();
                element.GetComponent<Choose_element>().set_param(topic_count, topic.Key, this, type, null, null, topic.Value);
            }
        }
        else if (type == 3)
        {
            Show_answers.instance.gameObject.SetActive(true);
            foreach (KeyValuePair<int, string[]> questions in get_topic(number_topic))
            {
                GameObject element = (GameObject)ChoosePool.Get();
                element.GetComponent<Choose_element>().set_param(questions.Key, null, this, type, answer_book, name_topic, questions.Value);
            }
        }
    }
    ...
    public void choose_action(int number, int type, string name)
    {
        ...
        Controller.instance.reset_button();
    }

    public async Task load()
    {
        Application.targetFrameRate = 60;
        await SaveSystem.load_from_file("last_position");
        Dictionary<string, string> last_pos = await SaveSystem.get_dict("last_position");
        if (last_pos.Count > 0)
        {
            set_book(int.Parse(await SaveSystem.get_value("last_position","book")));
            var questions = get_topic(int.Parse(await SaveSystem.get_value("last_position", "topic")));
            number_topic = int.Parse(await SaveSystem.get_value("last_position", "topic"));
            number_q = int.Parse(await SaveSystem.get_value("last_position", "question"));
            await SaveSystem.load_from_file(get_save_name());
            set_view(questions);
        }
    ...
    {
        check_current_color();
        Controller.instance.put_book_text(books[number_book]);
        Controller.instance.put_part_text(name_topic);
        Controller.instance.put_number_text("Вопрос "+number_q + "");
        Controller.instance.put_discription_text(questions[number_q][0].Insert(questions[number_q][0].IndexOf("а)") - 1, "\n"), new List<string>(answer_book[name_topic][number_q]));
        Controller.instance.put_answer(name_topic, number_q, questions[number_q], new List<string>(answer_book[name_topic][number_q]));
        save_last_view();
    }

    async void save_last_view()
    {
        ...
        await SaveSystem.save_to_file("last_position");
    }
    ...
    string get_save_name()
    {
     ...
     public async void check_current_color()
    {
        Controller.instance.set_color_book(await get_book_color(get_current_book_name(), number_book));
        Controller.instance.set_color_part(await get_topic_color(name_topic, get_topic(number_topic)));
        Controller.instance.set_color_number(await get_number_color(number_q));
    }
   ...
   public async Task<Color> get_book_color(string book_name, int book_number)
    {
        bool check = true;
        foreach (KeyValuePair <string, Dictionary<int, string[]>> topic in get_book_by_number(book_number))
        {
            var save = await SaveSystem.get_dict(create_save_name(book_name, topic.Key));
    ...
    public async Task<Color> get_topic_color(string topic_name, Dictionary<int, string[]> topic)
    {
        var save = await SaveSystem.get_dict(create_save_name(get_current_book_name(), topic_name));
        if (save.ContainsValue(0 + "")) return red;
        else if (topic.Count == save.Count) return green;
        else return black;
    }

    public async Task<Color> get_number_color(int number)
    {
        var save = await SaveSystem.get_dict(get_save_name());
    ...
    public void look_for(string text, bool next, TMP_Text count_text)
    {
        if (!text.Equals(""))
        {
            ...
            count_text.text = search_count+1 + "/" + search_mass.Count;
            if(search_mass.Count == 0) count_text.text = "";
        }
        else count_text.text = "";
    }
    private Object InstantiateObject()
    {
        Object obj = Instantiate(choose_element, parent);
        return obj;
    }
